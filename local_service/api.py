...
NER_ONLY_SYSTEM_PROMPT = """你是实体识别服务。对用户输入只输出 JSON，不要输出解释、推理过程或 <think> 标签。
识别以下标签：
t1: PERSON/ORG/LOCATION/PHONE/EMAIL/ID/MEDICAL_ID/IP/URL/ENTITY
t2: AGE/SALARY/AMOUNT/COUNT/WEIGHT/BLOOD_SUGAR/BLOOD_PRESSURE/MEDICAL_VAL/YEAR/DATE_NUM/NUM/PERCENT
输出格式：
{
  "entities": [{"token":"原文片段","label":"标签"}]
}"""
    ...
    def extract_entities(
        ...
        try:
            ...
            completion = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": NER_ONLY_SYSTEM_PROMPT},
