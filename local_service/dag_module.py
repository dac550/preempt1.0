        ...
        for ed in raw_edges:
            ...
            if param_value is not None:
                for token, entity in token_map.items():
                    if not entity.is_temp and abs(entity.value - actual_param) < 1e-9:
                        param_entity = entity
                        break

            # 处理每个父节点（单步操作，直接创建边）
            for p_entity in parent_entities:
                # 防止自环
                if p_entity.index == child_entity.index:
                    continue
            ...
            # 如果有参数实体，单独添加边
            if param_entity and param_entity not in parent_entities:
                if param_entity.index != child_entity.index:
                    parsed_edges.append(DAGEdge(
                        parent_idx=param_entity.index,
                        child_idx=child_entity.index,
                        rel_type=rel,  # 使用相同的关系类型
                        param=1.0,
                    ))
                    print(f"[DAG] 将 param 值 {actual_param} 对应的实体 '{param_entity.token}' 添加为父节点")
        # ============================================================================================
        # --- 去环 ---
